...
namespace Processor.UnitTests
{
	...
	public class FunctionalProcessorTests
	{
		...
		{
			var processor = new M6502.Processor();
		...
		{
            KdTestProgram = FunctionalProcessorTestData.FunctionalTestData;
			InterruptProgram = FunctionalProcessorTestData.InterruptTestData;
            CycleProgram = FunctionalProcessorTestData.CycleTestData;

            LoadCycleTestResults();
        }

        private void LoadCycleTestResults()
        {
            CycleTestDataResults = new List<TestData>();
            var lineNumber = 0;

			using (StringReader reader = new StringReader(FunctionalProcessorTestData.CycleTestResultsData))
            {
                while (true)
                {
